...
const KEEP_ALIVE =
  installConfig.keep_alive !== undefined ? installConfig.keep_alive : false;
const MOUNT_HOME_CHILDREN =
  installConfig.mount_home_children !== undefined
    ? installConfig.mount_home_children
    : true;
...
const PROJECT_SYMLINK_DEPTH =
  installConfig.project_symlink_depth !== undefined
    ? installConfig.project_symlink_depth
    : 3;
const FORWARD_SSH_AGENT =
  installConfig.forward_ssh_agent !== undefined
    ? installConfig.forward_ssh_agent
    : false;
const TOOL_PERMISSIONS = installConfig.tool_permissions || {
  allow: ["*"],
  deny: [],
};
...
function copyManaged(src, dest, label) {
  if (fs.existsSync(src)) {
    try {
      ...
      console.log(
        `[code-container] Warning: could not update ${label}: ${err.message}`,
      );
    }
  }
}
...
function findKnownHostsPath() {
  ...
  if (fs.existsSync("/etc/ssh/ssh_known_hosts"))
...
function findEnvPath(pattern, label) {
  ...
  const result = spawnSync(
    "find",
    [...searchRoots, "-maxdepth", "8", "-path", pattern, "-print", "-quit"],
    {
      encoding: "utf8",
      stdio: ["ignore", "pipe", "ignore"],
      timeout: ENV_SEARCH_TIMEOUT_MS,
    },
  );
...
if (isFirstInstall) {
  if (penvPath)
    console.log(`[code-container] Found Python environment: ${penvPath}`);
  if (renvPath)
    console.log(`[code-container] Found R environment: ${renvPath}`);
  if (!penvPath && !renvPath && (PENV_PATTERN || RENV_PATTERN)) {
    console.log(
      "[code-container] No Python/R environments found. Set penv_path and renv_path in ~/.code-container/config.json if needed.",
    );
  }
  if (knownHostsPath)
    console.log(`[code-container] Found SSH known_hosts: ${knownHostsPath}`);
  else
    console.log(
      "[code-container] SSH known_hosts not found. Set ssh_known_hosts_path in ~/.code-container/config.json if SSH agent forwarding is needed.",
    );
  ...
  try {
    ...
    console.log(
      `[code-container] Warning: could not parse ${HOST_CONFIG_PATH}: ${err.message} — skipping migration.`,
    );
    existing = null;
  }
  ...
  if (existing) {
    ...
    if (missing.includes("ssh_known_hosts_path")) {
      ...
      if (discovered)
        console.log(`[code-container] Found SSH known_hosts: ${discovered}`);
      else
        console.log(
          "[code-container] SSH known_hosts not found. Set ssh_known_hosts_path in ~/.code-container/config.json if SSH agent forwarding is needed.",
        );
    ...
    for (const k of missing)
      existing[k] =
        k in migrationOverrides ? migrationOverrides[k] : defaultConfig[k];
    for (const k of removed) delete existing[k];

    if (missing.length > 0 || removed.length > 0) {
      const removedEntries = removed.map(
        k => `${k} (was ${JSON.stringify(existing[k])})`,
      );
      fs.writeFileSync(
        HOST_CONFIG_PATH,
      ...
      fs.chmodSync(HOST_CONFIG_PATH, 0o600);
      console.log(
        `[code-container] Migrated config.json: added ${missing.length} keys, removed ${removed.length} keys`,
      );
      if (removedEntries.length > 0) {
        console.log(
          `[code-container] Deprecated keys removed: ${removedEntries.join(", ")}`,
        );
      }
    } else {
