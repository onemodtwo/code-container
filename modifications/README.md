...
> **Note:** `npm run build` must be run before every install or reinstall — the compiled `dist/` directory is not committed to the repo.
...
**Symlinks to writable locations are writable through the link.** If a symlink in a read-only mount (e.g. `/data/link-to-files → /writable/files`) resolves to a path that *is* mounted read-write (via `extra_readwrite`), writes through the symlink work — the kernel follows the link into the read-write mount. The read-only flag on the containing directory only prevents writes directly into that mount, not through symlinks that exit it.
