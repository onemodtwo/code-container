...
This document describes all modifications made to this fork of [code-container](https://github.com/kevinMEH/code-container) (upstream version 2.5.0). The fork is a substantial rework that replaces the multi-stage build pipeline with a single Dockerfile, replaces the text-file mount system with a security-hardened, configuration-driven mount system, and adds first-class support for enterprise/HPC environments (CephFS, Podman, SSH agent forwarding, shared R/Python environments).

The original is a general-purpose Docker harness for multiple AI coding tools (Claude, OpenCode, Codex, Copilot, Gemini). This fork retains multi-tool support while overhauling the configuration system, mount management, runtime support, and security model.
...
RUN ln -snf /usr/share/zoneinfo/$TZ /etc/localtime && echo $TZ > /etc/timezone \
    ...
    && npm install -g @anthropic-ai/claude-code @opencode-ai/cli @github/copilot-cli \
    && apt-get clean
```

Build-time parameters (`base_image`, `timezone`) are read from `config.json` and passed as `--build-arg` flags by the CLI.
...
12. Tool config files (settings, auth, history)
13. Git config
...
### Read-only tool settings

Tool settings files (e.g., Claude's `settings.json`) are mounted **read-only** inside the container. The agent cannot modify its own permissions during a session.

### Default permission deny list

The default permissions (from `install.json`) pre-approve all standard tools but deny explicit network-making shell commands:
...
## 6. Auth and History Modes

### Shared vs. per-project auth

Upstream copies tool config files into a per-project config directory at init time. This fork defaults to **shared** mode: auth files from the host are mounted directly (read-write, since tools may write to them on startup). Re-authentication on the host takes effect in all containers immediately.

Per-project mode is available by setting the relevant config option — useful for isolating OAuth tokens when different projects use different authentication.

### Shared vs. isolated history

Upstream isolates conversation history per container. This fork defaults to **shared** mode: the host's history directories are mounted directly, so conversation history is continuous across host and container sessions for the same project.
...
### Multi-stage build pipeline removed

The upstream four-stage build pipeline (`Dockerfile.Core` -> `Dockerfile.Packages` -> `Dockerfile.Harness` -> `Dockerfile.User`) and the `container build --target` argument are removed. Replaced by a single `Dockerfile` with build arguments as described in section 1.

### Config copy workflow replaced

Upstream copies tool config directories from `$HOME` into `~/.code-container/configs/` at init time, then mounts `configs/` into the container. This fork mounts config files directly from the host (in shared mode) or copies only auth files (in per-project mode). The `configs/` directory and the init-time copy logic are removed.
...
| `src/config.ts` | Removed multi-tool configs/copies; added per-project directory management, path canonicalization, permissions injection |
...
| `Dockerfile` | Single file; Ubuntu base; build args; minimal packages; runtime bashrc mount |
...
| `package.json` | Simplified; removed unused deps; added `engines` field |
